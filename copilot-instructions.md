...
## Project Overview
A secure, modular desktop app using Tauri v2 (Rust), React, and Python (FastAPI).
...
- **Rule:** NEVER hardcode ports (8000). Always use `os.environ["API_PORT"]`.

### 3. Frontend (`src/`)
- **Pattern:** Logic/View Separation.
- **`hooks/`**: All side-effects (Process spawning, Event listening) MUST go into custom hooks.
...
## 5. Testing Mandate (Zero Tolerance)
**You must generate tests for every new feature.**

- **New React Component?**
    - Create `src/components/MyComponent.test.tsx`.
    - Use `render`, `screen`, and `@testing-library/react`.
- **New Python Endpoint?**
    - Create `python-backend/tests/test_feature.py`.
    - Use `TestClient` and check for 403 (Forbidden) vs 200 (OK).
- **New Rust Logic?**
    - Add `#[test]` mod inside `src/lib.rs` or the specific module.
